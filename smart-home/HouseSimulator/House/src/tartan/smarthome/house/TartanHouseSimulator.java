...
public class TartanHouseSimulator implements Runnable {
    ...
    private Boolean proximityState; // the state of the proximity sensor (true if house occupied, false if vacant)
    private Boolean brokenWindowState; // the state of the window sensor (true if a window is broken, false if all
                                       // windows are intact)
    private Boolean intruderState; // the state of the intruder defense system (true if the system thinks there is
                                   // an intruder, false if there is no intruder)
    ...
    private Boolean geofenceState; // the geofence state (true if geofence is active, false if geofence is
                                   // inactive)
    private Boolean allClear; // the all clear state (true if all clear, false if not all clear)
    /** connection settings */
    ...
    private final String BROKEN_WINDOW = "BW";
    ...
    private final String GEOFENCE_STATE = "GFS";
    private final String ALL_CLEAR = "AC";
    ...
    public synchronized Boolean setState(Hashtable<String, Object> state) {
        ...
        for (String key : keys) {
            ...
            } else if (key.equals(GEOFENCE_STATE)) {
                Boolean newGeofenceState = (Boolean) state.get(key);
                newState.append(GEOFENCE_STATE);
                newState.append(PARAM_EQ);
                if (newGeofenceState) {
                    newState.append(GEOFENCE_ON);
                } else {
                    newState.append(GEOFENCE_OFF);
                }
                count++;
                if (count < keys.size()) {
                    newState.append(PARAM_DELIM);
                }
            } else if (key.equals(ALL_CLEAR)) {
                Boolean newAllClear = (Boolean) state.get(key);
                newState.append(ALL_CLEAR);
                newState.append(PARAM_EQ);
                if (newAllClear) {
                    newState.append(ALL_CLEAR_ON);
                } else {
                    newState.append(ALL_CLEAR_OFF);
                }
                count++;
                if (count < keys.size()) {
                    newState.append(PARAM_DELIM);
    ...
    private Hashtable<String, Object> handleStateUpdate(String stateUpdateMsg) {
        ...
        while (pt.hasMoreTokens()) {
            ...
            } else if (data[0].equals(BROKEN_WINDOW)) {
                if (val == 1) {
                    state.put(BROKEN_WINDOW, true);
                } else {
                    state.put(BROKEN_WINDOW, false);
                }
            ...
            } else if (data[0].equals(GEOFENCE_STATE)) {
                if (val == 1) {
                    state.put(GEOFENCE_STATE, true);
                } else {
                    state.put(GEOFENCE_STATE, false);
                }
            } else if (data[0].equals(ALL_CLEAR)) {
                if (val == 1) {
                    state.put(ALL_CLEAR, true);
                } else {
                    state.put(ALL_CLEAR, false);
                }
