...
class HouseState(object):
   ...
   def __init__(self):
      ...
      self.__broken_window = False
      self.__intruder = False
      self.__geofence = False
   ...
   def set_state(self, new_state):
      ...
      for par in params:
         ...
         if k == "LS":
            ...
            if v == "1": self.__broken_window = True
            else: self.__broken_window = False
         elif k == "IS":
            if v == "1": self.__intruder = True
            else: self.__intruder = False
         elif k == "GFS":
            if v == "1": self.__geofence = True
            else: self.__geofence = False
         elif k == "AC":
            if v == "1": self.__allclear = True
            else: self.__allclear = False
         elif k == "HES":
   ...
   def set_broken_window(self, p):  self.__broken_window = p
   def get_broken_window(self):
      if self.__broken_window: return "1"
      return "0"
   
   def set_intruder(self, p):  self.__intruder = p
   def get_intruder(self):
      if self.__intruder: return "1"
      return "0"
   
   def set_geofence(self, p):  self.__geofence = p
   def get_geofence(self):
      if self.__geofence: return "1"
      return "0"
   
   def set_allclear(self, p):  self.__allclear = p
   def get_allclear(self):
      if self.__allclear: return "1"
      return "0"
   ...
   def get_state(self):
      ...
      '''
      return "TR={0};HR={1};DS={2};LS={3};PS={4};BW={5};IS={6};AS={7};AA={8};HES={9};CHS={10};HM={11};HUS={12};GFS={13};AC={14}".format(
                                                                                             self.get_temperature(),
                                                                                               self.get_humidity(),
                                                                                               self.get_door(),
                                                                                               self.get_light(),
                                                                                               self.get_proximity(),
                                                                                               self.get_broken_window(),
                                                                                               self.get_intruder(),
...
class UserThread(threading.Thread):
   ...
   def run(self):
      ...
      while not self.__stop.is_set():
         ...
         cmd = input("Enter a command: d=[toggle door], l=[toggle light], p=[toggle proximity], b=[toggle broken window state], i=[toggle intruder state], RET=[show current status]: ")
         ...
         if cmd == "d":
            ...
            if house.get_broken_window() == "1": house.set_broken_window(False)
            else: house.set_broken_window(True)
            print ("Broken window state is now {}".format(house.get_broken_window()))
         elif cmd == "i":
            if house.get_intruder() == "1": house.set_intruder(False)
            else: house.set_intruder(True)
            print ("Intruder state is now {}".format(house.get_intruder()))
